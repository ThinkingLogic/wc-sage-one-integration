...
class SageoneClient {
  ...
  */
  ...
  public function getData($endpoint, $header) {
    ...
    ThinkingLogicWCSage::log("Called GET endpoint " . $endpoint . " with headers " . json_encode($header) . ", response: " . $response);
  ...
  public function postData($endpoint, $post_data, $header) {
    ...
    $response = curl_exec($curl);
    ThinkingLogicWCSage::log("Called POST endpoint " . $endpoint . " with headers " . json_encode($header) . " and body '" . json_encode($post_data) . "', response: " . $response);
  ...
  public function putData($endpoint, $put_data, $header) {
    ...
    $response = curl_exec($curl);
    if (!$response) { ThinkingLogicWCSage::log("Error response from " . $endpoint . " using header " . $header); } 
  ...
  private function getToken($params) {
    ...
    $options = array('http' => array('header'  => "Content-type: application/x-www-form-urlencoded",
                                     'method'  => 'POST',
                                     'content' => $this->constructParameters($params)));
    $context  = stream_context_create($options);
    $result = file_get_contents($url, false, $context);
    if (!$result) { 
        ThinkingLogicWCSage::log("Unable to get token from url " . $url . " with options " . json_encode($options));
    }
